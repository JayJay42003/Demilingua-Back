...
public class ProgressService {
    ...
    public Map<String, String> saveTestProgress(int usuarioId, int testId, int puntuacion) {
        ...
        try {
            ...
            if (ut.getId() == null) {
                ut.setId(id);
                ut.setUsuario(usuarioRepository.findById(usuarioId).orElseThrow(() -> new NoSuchElementException("Usuario no encontrado")));
                ut.setTest(testRepository.findById(testId).orElseThrow(() -> new NoSuchElementException("Test no encontrado")));
