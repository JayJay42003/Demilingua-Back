...
public class FriendshipService {
    ...
    public List<Map<String, String>> getFriends(int usuarioId) {
        ...
        for (Amistad a : friendships) {
            ...
            // Esto asume que Usuario tiene una lista de UsuarioIdioma o lo buscamos directamente
            // Por simplicidad en este paso, lo calculamos manual si no hay relación en la entidad
            // En una app real, usaríamos una consulta agregada en UsuarioIdiomaRepository
            row.put("puntos", "0"); // TODO: Implementar suma de puntos real si es necesario
            result.add(row);
        }
        return result;
    }
    ...
    public Map<String, String> addFriend(int id1, int id2) {
        ...
        try {
            ...
            amistad.setId(id);
            amistad.setUsuario1(usuarioRepository.findById(id1).orElseThrow());
            amistad.setUsuario2(usuarioRepository.findById(id2).orElseThrow());
